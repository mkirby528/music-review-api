...
from src.users.get_user import get_user_by_id
...
def lambda_handler(event, context):
    ...
    if method == "GET" and re.fullmatch(constants.GET_ALL_ALBUMS_PATH_REGEX, path):
        ...
        response = get_all_reviews(review_table, query_params)
    ...
    if method == "GET" and re.fullmatch(constants.GET_ALBUM_BY_ID_PATH_REGEX, path):
        ...
        response = get_review_by_id(review_table, album_id)
        return format_response(200, response)
    ...
    if method == "POST":
        ...
        return format_response(201, add_album_to_db(review_table, album_item))
    if method == "PATCH":
        album_id = path_paramaters.get("albumID", "")
        if not album_id:
        ...
        if bool(re.match(constants.ADD_VINYL_PATH_REGEX, path)):
            return format_response(200, addVinylRecord(review_table, album_id))
        # PATCH /albums/:albumId
        else:
            update_fields = json.loads(body)
            album = update_album(review_table, album_id, update_fields)
            return format_response(200, album)
    if method == "DELETE":
        album_id = path_paramaters.get("albumID", "")
        if not album_id:
            return (400, "No albumID provided.")
        response = review_table.delete_item(
            Key={
                'id': album_id
            }
        )
    ...
    ############################### USER ENDPOINTS ###############################
    print(re.match(constants.GET_USER_BY_ID_PATH_REGEX, path))
    if method == "GET" and re.fullmatch(constants.GET_USER_BY_ID_PATH_REGEX, path):
        user_id = path_paramaters.get("userId", "")
        print(user_id)
        if not user_id:
            return format_response(404, {})
